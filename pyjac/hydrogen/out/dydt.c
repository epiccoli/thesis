...
void dydt (const double t, const double pres, const double * __restrict__ y, double * __restrict__ dy) {
  ...
  dy[0] = (-1.0 / (rho * cp_avg)) * ((dy[1] * h[0] * 2.0158800000000001e+00)
        + (dy[2] * h[1] * 3.1998799999999999e+01) + (dy[3] * h[2] * 1.8015280000000001e+01)
        + (dy[4] * h[3] * 1.0079400000000001e+00) + (dy[5] * h[4] * 1.5999400000000000e+01)
        + (dy[6] * h[5] * 1.7007339999999999e+01) + (dy[7] * h[6] * 3.3006740000000001e+01)
  ...
  dy[3] *= (1.8015280000000001e+01 / rho);
...
void dydt (const double t, const double rho, const double * __restrict__ y, double * __restrict__ dy) {
  ...
  dy[0] = (-1.0 / (rho * cv_avg)) * ((dy[1] * u[0] * 2.0158800000000001e+00)
        + (dy[2] * u[1] * 3.1998799999999999e+01) + (dy[3] * u[2] * 1.8015280000000001e+01)
        + (dy[4] * u[3] * 1.0079400000000001e+00) + (dy[5] * u[4] * 1.5999400000000000e+01)
        + (dy[6] * u[5] * 1.7007339999999999e+01) + (dy[7] * u[6] * 3.3006740000000001e+01)
